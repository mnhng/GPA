...
def run_experiment(args):
    ...
    need_cond_prev = args.method in {'copa', 'gpa'}

    d_train = [args.env(b, b, seed=args.seed, **ptrain)(n_tr) for b, n_tr in conf['train']]
    d_valid = [args.env(b, b, seed=args.seed, **pvalid)(n_va) for b, n_va in [conf['valid']]]
    d_test = [args.env(b, b, seed=args.seed, **ptest)(n_tt) for b, n_tt in conf['test']]
    ...
    Zs = [core.SIM_Z_DICT['color']]
    setattr(args, 'Zs', Zs)

    trunk = core.get_trunk('ora' if args.method == 'ora' else 'cnn')(args.hidden_dim)
    ...
    row = {'N': args.method} | core.risk_round(model, d_train+d_test, args.batch_size, 'end')

    model.load_state_dict(best['state_dict'][0])

    rows = []
    if args.method == 'copa':
        rows.append(row | core.risk_round(model, d_test, args.batch_size, {'prior': 'true_prior'}))
        ...
        core.get_g_estimate_using_unlabeled_data(model, d_test)

        rows.append(row | core.risk_round(model, d_test, args.batch_size, {'prior': 'em_prior'}))
        rows.append({'N': f'{args.method}_ast'} | core.risk_round(model, d_test, args.batch_size, {'prior': 'em_prior', 'no_Z': True}))

    else:
        rows.append(row | core.risk_round(model, d_test, args.batch_size))
