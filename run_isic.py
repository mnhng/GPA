...
def run_experiment(args):
    ...
    d_train = [core.ISIC_site(img_dir, c, args.zlabels, fit_cond_prev=adjustment) for c in conf['train']]
    d_valid = [core.ISIC_site(img_dir, conf['valid'], args.zlabels, fit_cond_prev=adjustment)]
    d_test = [core.ISIC_site(img_dir, c, args.zlabels, fit_cond_prev=adjustment) for c in conf['test']]
    ...
    Zs = [core.ISIC_Z_DICT[zl] for zl in args.zlabels]
    setattr(args, 'Zs', Zs)

    trunk = core.get_trunk('r50')(args.hidden_dim)
    head = core.get_head(args.method)(args.hidden_dim, len(core.ISIC_Y), Zs)
    ...
    if args.method == 'copa':
        ...
        core.get_g_estimate_using_unlabeled_data(model, d_test, akl=core.ISIC_AKL)
